...
class UserFollowGenreService {
  ...
  Future<bool> isFollowingGenre(String genreId) async {
    final String response = await rootBundle.loadString('assets/databases/join_table/user_follow_genre.json');
    final List<dynamic> followJson = json.decode(response) as List<dynamic>;

    return followJson.any((follow) => follow['userId'] == userId && follow['genreId'] == genreId);
  }
  ...
  Future<void> unfollowGenre(String genreId) async {
    ...
    followJson.removeWhere((follow) => follow['userId'] == userId && follow['genreId'] == genreId);
  ...
  Future<List<Genre>> getFollowedGenresByUserId(String userId) async {
    ...
    return allGenres.where((genre) => followedGenreIds.contains(genre.id)).toList();
  }
}
